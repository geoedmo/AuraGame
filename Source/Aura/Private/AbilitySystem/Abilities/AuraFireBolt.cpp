...
FString UAuraFireBolt::GetNextLevelDescription(int32 Level)
{
	...
	const int32 Damage = GetDamageByDamageType(Level, FAuraGameplayTags::Get().Damage_Fire); // Was ( DamageTypes[FAuraGameplayTags::Get().Damage_Fire].GetValueAtLevel(Level); )
	const float ManaCost = FMath::Abs(GetManaCost(Level));
	const float Cooldown = GetCooldown(Level);
